...
def create_tables():
    ...
    with psycopg2.connect(db) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.NamedTupleCursor) as cur:
            ...
            cur.execute("CREATE TABLE IF NOT EXISTS reminder_schedule ( \
                reminder_id INTEGER, \
                day INTEGER, \
                hour INTEGER, \
                minute INTEGER, \
    ...
    with psycopg2.connect(db) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.NamedTupleCursor) as cur:
            ...
            for reminder in weekly:
                ...
                cur.execute(f"INSERT INTO reminder_schedule \
                    (reminder_id, day, hour, minute) \
                    VALUES (1, {reminder[0]}, {reminder[1]}, {reminder[2]}) \
                ")
...
def drop_tables():
    ...
    with psycopg2.connect(db) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.NamedTupleCursor) as cur:
            ...
            cur.execute("DROP TABLE IF EXISTS reminder_schedule")
    conn.close()
